...
local의 dev 브랜치에서 작업을 하고 원격 브랜치인 dev에 push를 하였다.
...
현재 로컬 dev 브랜치와 원격 dev 브랜치가 서로 다른 이력을 가지고 있어, Git이 어떤것을 합쳐야할지 결정을 내리지 못해 발생한 문제이다.

해결방법으로는 `Merge` 와 `Rebase`가 있다.
`Fast-forward`도 있다는데 사용해보지는 않았다.
