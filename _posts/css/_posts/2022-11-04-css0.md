...
`static`은 기본값으로 아무런 값을주지 않아 요소를 문서 흐름에 따라 배치합니다.  
...
`absolute`는 브라우저가 문서의 흐름과 상관없이 left, right, top, bottom 속성값을 이용하여 요소를 위치시키는 속성값입니다.  
...
`fixed`는 absolute 속성값과 마찬가지로 문서의 흐름과 상관없이 위치를 좌표로 결정합니다.  
...
`sticky`는 기준점을 이상을 넘지 않을 때는 relative 포지션처럼 동작하다 그 이상을 넘게 될 시에는 fixed처럼 동작합니다.  
...
반면에 `sticky`요소는 특정 시점에서 fixed 요소처럼 행동하다 자신의 부모 요소와 만나게 되면 멈추게 됩니다.  
...
`inline`요소는 불바꿈 없이 바로 옆에 생성됩며, 화면에 표시되는 영역만큼만 차지합니다.  
