...
인증 손상이란 웹 애플리케이션의 인증 메커니즘이 취약하여 공격자가 불법적으로 계정에 접근할 수 있는 보안 문제를 의미한다.  
 이는 잘못된 인증 구현, 약한 비밀번호 정책, 인증 우회 등의 원인으로 발생할 수 있다.

## 인증 구현 (Authentication Implementation)

웹 애플리케이션에서 인증은 사용자의 신원을 확인하는 과정이며, 다양한 방식으로 구현할 수 있다.

### 1. HTTP 네이티브 인증(HTTP Native Authentication)

HTTP 네이티브 인증은 브라우저와 서버 간의 기본적인 인증 방식으로, HTTP 프로토콜에서 제공하는 인증 메커니즘을 활용한다.  
...
사용자 대신 **OAuth 제공자(Google 등)**가 인증을 수행하며, 액세스 토큰을 발급하여 애플리케이션이 사용자 정보를 가져올 수 있도록 한다.
...
**무작위 대입 공격(Brute-Force Attack)**은 공격자가 가능한 모든 조합의 아이디와 비밀번호를 시도하여 인증을 우회하는 공격 방식이다.
...
**일반 무작위 대입 공격**
모든 가능한 문자 조합을 시도하여 비밀번호를 찾는 방식.
...
**1. 계정 잠금:** 일정 횟수 이상 로그인 실패 시 계정을 일시적으로 잠금.
**2. CAPTCHA 사용:** 비정상적인 로그인 시도를 방지.
**3. 비밀번호 복잡성 요구:** 영문, 숫자, 특수문자를 조합한 비밀번호 사용.
**4. 2FA (Two-Factor Authentication, 이중 인증):** 로그인 시 추가적인 인증을 요구.
...
**서드파티 인증**은 Google, Facebook, Apple 등의 외부 인증 제공자를 활용하는 방식입니다.
OAuth 2.0, OpenID Connect(OIDC) 등을 사용하여 사용자 인증을 수행하며, 애플리케이션은 인증을 직접 구현하지 않고, 외부 제공자에게 위임한다.

- ✅ 장점

  - 자체 인증 시스템을 구축할 필요가 없어 보안성을 향상시킴.
...
- ❌ 단점
  ...
  - 인증 시스템이 OAuth 제공자의 장애에 영향을 받을 수 있음.
...
**Single Sign-On(SSO)**은 한 번의 로그인으로 여러 서비스에 접근할 수 있도록 하는 인증 시스템입니다.
대표적인 SSO 프로토콜은 **SAML(Security Assertion Markup Language)**과 OAuth/OpenID Connect가 있습니다.

✅ 장점

사용자는 한 번만 로그인하면 여러 서비스에 접근할 수 있음.
조직 내에서 통합된 인증 정책을 적용할 수 있음.
비밀번호 관리 부담 감소.
❌ 단점

SSO 제공자가 장애를 일으키면 모든 서비스 로그인 불가.
...
SSO는 대기업, SaaS 플랫폼 등에서 많이 사용되며 보안성과 관리 효율성을 높일 수 있습니다.
...
암호화되지 않은 비밀번호 저장은 심각한 보안 위험을 초래합니다.
따라서 해싱 및 솔팅 기법을 적용하여 안전하게 저장해야 합니다.

**1. 해싱 암호 (Password Hashing)**

비밀번호를 데이터베이스에 저장할 때, 일방향 해시 함수를 사용하여 원래 값을 알 수 없도록 만듭니다.
일반적인 해싱 알고리즘: bcrypt, argon2, PBKDF2 등.

**2. 솔팅 해시 (Salting Hash)**

단순 해싱만 하면 동일한 비밀번호는 동일한 해시 값을 가지므로 솔트(Salt) 값을 추가하여 보안을 강화합니다.
...
사용자 로그인 시 **2단계 인증(2FA)**을 적용하여 계정 보안을 강화.  
 ex) 비밀번호 + SMS 인증 코드, 비밀번호 + OTP(Google Authenticator).
...
API 응답을 통일하여 사용자 정보 유출 방지.  
 ex) "로그인 정보를 확인하세요"와 같이 모든 실패 응답을 동일하게 설정.
...
- **서드파티 인증(OAuth, OpenID Connect)** 을 사용하면 보안성을 높이고, 인증 관리를 위임할 수 있음.
- SSO를 활용하면 사용자 경험을 개선하고, 여러 서비스 간 인증을 효율적으로 관리할 수 있음.
- 자체 인증 시스템을 구축할 경우, 안전한 비밀번호 정책, 해싱 암호화, MFA, CAPTCHA 등을 적용하여 보안을 강화해야 함.
