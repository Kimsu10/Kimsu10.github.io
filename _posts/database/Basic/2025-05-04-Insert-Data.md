...
- 테이블에 새로운 데이터를 입력할때는 `INSERT`문을 사용한다.
    - `숫자`: 그대로 기술
...
- **()를 쓰지않는 경우**
    ...
    INSERT INTO EMP (EMPNO, ENAME) VALUES (2, 'JEONG');
    ```
...
- <u>컬럼 값을 생략</u>하면 자동으로 `NULL` 이 들어간다
...
> '' 를 NULL 로 사용시 주의점
> - <u>Oracle의 경우</u> '' (빈 문자열)을 <u>NULL로 간주</u>한다  
> (`'' == NULL`)
> - <u>MySQL의 경우</u> '' (빈 문자열)은 그 자체로 <u>값이 있는 문자열</u>로 간주한다  
> (`'' != NULL`)
>
> | DB         | `''` 처리   | `''` = `NULL`? |
>| ---------- | --------- | -------------- |
