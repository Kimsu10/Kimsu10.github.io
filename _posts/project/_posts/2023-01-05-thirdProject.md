...
1. puppeteer를 사용하여 브라우저를 실행하고 새 페이지를 생성하기
...
2. 입력받은 광고의 url이 인스타그램인지 네이버블로그인지에따라 검색 파라미터를 처리하고 해당하는 URL을 반환하기

이코드를 작성할때 어려웠던점은 json 파일이였다.
json 파일로 데이터는 이렇게 되어있습니다라고 말씀하셨는데  
이걸 어떻게 활용해야할지 몰라서 애먹었다.
...
5.그 밖의 설정들
처음 작동시켰을때 사이트에 들어가자마자 바로 다음키워드를 검색하는 창으로 넘어갔다.  
그래서 페이지에 이동하고 3초간 대기, 및 위에 만든 함수들을 실행되게 하였다.  
...
초기에 회사와 학원간의 날짜착오와 기획의 부족으로 일주일이라는 시간을 사용하여
실제 작업에 착수한 기간은 17일 정도였고, 간단한 작업을 주셨다고 생각했다.  
