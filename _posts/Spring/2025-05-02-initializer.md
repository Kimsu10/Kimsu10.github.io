...
**1. 위 페이지에서 자신이 사용할 환경에 맞게 설정한 후 generate를 눌러 압축파일을 다운로드**
...
- 압축 해제 후 setting.gradle에서 의존성을 확인하고 추가 할 수 있다
...
- Spring Initializr로 생성한 프로젝트의 압축을 해제한 뒤 `ls` 명령어로 `build/libs` 디렉터리를 살펴보면,  
  아직 빌드가 실행되지 않았기 때문에 `.jar` 파일은 존재하지 않는다

- 초기 상태에서는 `build.gradle` 파일에 기본적으로 `version = '0.0.1-SNAPSHOT'`이 설정되어 있지만,  
  이 설정은 파일 이름으로 바로 드러나지는 않는다
